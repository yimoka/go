...
var dfMsgMap = map[MsgKey]*i18n.Message{
	...
	missingMetadataKey:           {ID: missingMetadataKey.String(), Other: "Missing metadata {{.Name}}, please check whether the transmission link is enabled for metadata transmission and pass the value"},
...
var ruMsgMap = map[MsgKey]*i18n.Message{
	...
	missingMetadataKey:           {ID: missingMetadataKey.String(), Other: "Отсутствует метаданные {{.Name}}, пожалуйста, проверьте, включена ли передача метаданных в цепи передачи и передайте значение"},
